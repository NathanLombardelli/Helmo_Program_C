...
int main()
{
    ...
    default: /*processus père*/
        close(1); /*ferme l'entrer stdout*/
        dup(tube[ECRITURE]);/*duplique le descripteur en écriture du tube dans cettee entréé (toute les productions a l'écran sont envoyéesdans le tube)*/
        close(tube[LECTURE]);
        close(tube[ECRITURE]);
